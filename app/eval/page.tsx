...
export default async function EvalPage({
    ...
    return (
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <p className="text-text-muted">
          No evaluation run found. Run <code className="rounded bg-border-soft px-1 font-mono text-sm">npm run eval:seed</code> then{" "}
          <code className="rounded bg-border-soft px-1 font-mono text-sm">npm run eval:run</code> to generate results.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4" data-testid="eval-metrics">
        ...
        <Card>
          ...
          <CardContent>
            <p className="font-mono text-lg font-semibold text-text-primary">
              {metrics.retrieval
                ? `${(metrics.retrieval.policyHitRate * 100).toFixed(1)}%`
                : "—"}
            </p>
        ...
        <Card>
          ...
          <CardContent>
            <p className="font-mono text-lg font-semibold text-text-primary">
              {metrics.retrieval
                ? `${(metrics.retrieval.precedentHitRate * 100).toFixed(1)}%`
                : "—"}
            </p>
      ...
      {run.summary && (
        <p className="mt-6 text-sm text-text-muted">{run.summary}</p>
      )}
    </div>
  );
}
